...
let textObjInfo = []
const backendURL = 'http://localhost:8888/v1/recording/store-subtitle'; // Replace with your backend URL
...
function startRecordingOnServer(textObjInfo) {
    const url = backendURL; // Use the base URL

    const requestBody = {
        textObjInfo: textObjInfo // Include the serverId in the request body
        // Add other required parameters to the requestBody if needed
    };
    ...
    fetch(url, {
        ...
        .then(response => {
            if (response.ok) {
                return response.json(); // Parse response JSON if expecting JSON data
            }
            throw new Error('Network response was not ok.');
        })
        .then(data => {
            // Handle response data from the backend if needed
    ...
    try {
        ...
        call.on('stateChanged', async () => {
            if(call.state === 'Connected') {
                ...
                startRecordingOnServer(textObjInfo)
        ...
        call.remoteParticipants.forEach(remoteParticipant => {
            console.log(remoteParticipant,"line380")
